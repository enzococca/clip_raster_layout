...
class ClipRasterDialog(QDialog):
    ...
    def setupUi(self):
        ...
        # Add to map checkbox
        self.add_to_map_check = QCheckBox(
            "Aggiungi raster clippati alla mappa")
    ...
    def run_clip(self):
        ...
        if not polygon_layer:
            QMessageBox.warning(self, "Attenzione",
                                "Seleziona un poligono di clip")
            return

        selected_items = self.raster_list.selectedItems()
        if not selected_items:
            QMessageBox.warning(self, "Attenzione",
                                "Seleziona almeno un raster")
            return
        ...
        self.progress_label.setText(
            f"Clipping 0/{len(selected_items)} raster...")
        ...
        for i, item in enumerate(selected_items):
            ...
            output_path = os.path.join(
                output_folder, f"clipped_{raster_layer.name()}.tif")
            ...
            try:
                ...
                if self.add_to_map_check.isChecked() and result['OUTPUT']:
                    clipped_layer = QgsRasterLayer(
                        result['OUTPUT'], f"clipped_{raster_layer.name()}")
                    if clipped_layer.isValid():
                        ...
                        if 'dem' in layer_name or 'dtm' in layer_name or 'dsm' in layer_name:
                            dem_layers.append(
                                (clipped_layer, raster_layer.name()))
                        else:
                            ortho_layers.append(
                                (clipped_layer, raster_layer.name()))
                ...
                QMessageBox.warning(
                    self, "Errore", f"Errore nel clipping di {raster_layer.name()}: {str(e)}")

            self.progress_bar.setValue(i + 1)
            self.progress_label.setText(
                f"Clipping {i + 1}/{len(selected_items)} raster...")
