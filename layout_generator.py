...
from qgis.PyQt.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                                 ...
                                 QTextEdit, QFileDialog, QDockWidget, QWidget)
from qgis.core import (QgsProject, QgsPrintLayout, QgsLayoutItemMap, QgsLayoutItemLabel,
                       QgsLayoutItemScaleBar, QgsLayoutItemPicture, QgsLayoutItemLegend,
                       QgsLayoutPoint, QgsLayoutSize, QgsUnitTypes,
                       QgsLayoutExporter, QgsLayoutItemShape, QgsVectorLayer,
                       QgsRasterLayer, QgsLayoutMeasurement,
                       QgsLayoutItem, QgsLayoutItemMapOverview, QgsScaleBarSettings,
                       QgsMessageLog, Qgis,
                       QgsWkbTypes, QgsLayoutObject,
                       QgsProperty)
...
class LayoutGenerator(QDialog):
    ...
    def generate_layout(self):
        ...
        if self.template_checkbox.isChecked():
            ...
            template_path = os.path.join(
                os.path.dirname(__file__), "template_layout.qpt")
            if os.path.exists(template_path):
                ...
                if layout:
                    ...
                    QMessageBox.information(
                        self, "Successo", "Layout generato con successo dal template!")
                    return
                else:
                    QgsMessageLog.logMessage(
                        "Failed to load template, creating new layout", "ClipRasterLayout", Qgis.Warning)
        ...
        page.setPageSize(QgsLayoutSize(
            size[0], size[1], QgsUnitTypes.LayoutMillimeters))
        ...
        map_item.setFrameStrokeWidth(QgsLayoutMeasurement(
            0.5, QgsUnitTypes.LayoutMillimeters))
        ...
        title.attemptMove(QgsLayoutPoint(
            size[0]/2 - 50, 4, QgsUnitTypes.LayoutMillimeters))
        title.attemptResize(QgsLayoutSize(
            100, 18, QgsUnitTypes.LayoutMillimeters))
        ...
        if self.include_north.isChecked():
            ...
            north.attemptMove(QgsLayoutPoint(
                12.7, 18.9, QgsUnitTypes.LayoutMillimeters))
            north.attemptResize(QgsLayoutSize(
                8.7, 11.6, QgsUnitTypes.LayoutMillimeters))
            north.setFrameEnabled(True)
            north.setFrameStrokeWidth(QgsLayoutMeasurement(
        ...
        if self.include_scale.isChecked():
            ...
            if self.paper_combo.currentText() == "A3 Landscape":
                scalebar.attemptMove(QgsLayoutPoint(
                    88.96, 150.16, QgsUnitTypes.LayoutMillimeters))
            else:
                scalebar.attemptMove(QgsLayoutPoint(
                    20, size[1] - 40, QgsUnitTypes.LayoutMillimeters))

            scalebar.setFrameEnabled(True)
            scalebar.setFrameStrokeWidth(QgsLayoutMeasurement(
                0.3, QgsUnitTypes.LayoutMillimeters))
        ...
        if self.include_legend.isChecked():
            ...
            if self.paper_combo.currentText() == "A3 Landscape":
                legend.attemptMove(QgsLayoutPoint(
                    209.9, 70, QgsUnitTypes.LayoutMillimeters))
                legend.attemptResize(QgsLayoutSize(
                    80, 30, QgsUnitTypes.LayoutMillimeters))
            else:
                legend.attemptMove(QgsLayoutPoint(
                    size[0] * 0.75, 70, QgsUnitTypes.LayoutMillimeters))
                legend.attemptResize(QgsLayoutSize(
                    size[0] * 0.2, 40, QgsUnitTypes.LayoutMillimeters))
        ...
        if self.include_overview.isChecked():
            ...
            if self.paper_combo.currentText() == "A3 Landscape":
                ...
                overview.setRect(size[0] * 0.75, 20,
            ...
            overview.setFrameStrokeWidth(QgsLayoutMeasurement(
                0.5, QgsUnitTypes.LayoutMillimeters))
        ...
        if self.include_profile.isChecked() and self.profile_combo.currentData():
            ...
            if HAS_ELEVATION_PROFILE:
                try:
                    ...
                    QgsMessageLog.logMessage(
                        "Using native QGIS elevation profile", "ClipRasterLayout", Qgis.Info)
                except Exception as e:
                    QgsMessageLog.logMessage(
                        f"Native profile failed: {e}, using custom matplotlib method", "ClipRasterLayout", Qgis.Warning)
                    self.add_profile_graph(layout, size)
            else:
                # Use custom method for older QGIS versions
                QgsMessageLog.logMessage(
                    "QGIS elevation profile not available, using custom matplotlib method", "ClipRasterLayout", Qgis.Info)
                self.add_profile_graph(layout, size)
        ...
        if self.logo_path.text():
            ...
            logo.attemptMove(QgsLayoutPoint(
                size[0] - 50, size[1] - 40, QgsUnitTypes.LayoutMillimeters))
            logo.attemptResize(QgsLayoutSize(
                40, 30, QgsUnitTypes.LayoutMillimeters))
        ...
        QMessageBox.information(
            self, "Successo", "Layout generato con successo!")
    ...
    def add_native_elevation_profile(self, layout, page_size):
        ...
        if self.paper_combo.currentText() == "A3 Landscape":
            profile_item.attemptMove(QgsLayoutPoint(
                3.99, 168.75, QgsUnitTypes.LayoutMillimeters))
            profile_item.attemptResize(QgsLayoutSize(
                287.19, 38.87, QgsUnitTypes.LayoutMillimeters))
        else:
            profile_item.attemptMove(QgsLayoutPoint(
                10, page_size[1] - 50, QgsUnitTypes.LayoutMillimeters))
            profile_item.attemptResize(QgsLayoutSize(
                page_size[0] - 20, 40, QgsUnitTypes.LayoutMillimeters))
        ...
        if dem_layer:
            try:
                ...
                try:
                    ...
                    layer_settings = layers.profileLayerSettings(
                        dem_layer.id())
                    if layer_settings:
                        layer_settings.setEnabled(True)
                        layers.setProfileLayerSettings(
                            dem_layer.id(), layer_settings)
                except Exception:
                    # Fallback - just add the layer
                    QgsMessageLog.logMessage(
                        "Could not configure elevation profile layers, using default settings", "ClipRasterLayout", Qgis.Warning)
        ...
        profile_item.setFrameStrokeWidth(
        ...
        QgsMessageLog.logMessage(
            f"Native elevation profile added for {profile_name}", "ClipRasterLayout", Qgis.Info)
    ...
    def add_profile_graph(self, layout, page_size):
        ...
        if not profile_feature:
            QgsMessageLog.logMessage(
                "No profile feature selected", "ClipRasterLayout", Qgis.Warning)
            return
        ...
        QgsMessageLog.logMessage(
            f"Adding profile: {profile_name}", "ClipRasterLayout", Qgis.Info)
        ...
        profile_path, _ = QgsProject.instance().readEntry(
            "ClipRasterLayout", f"profile_{profile_name}")
        ...
        if not profile_path or not os.path.exists(profile_path):
            ...
            if save_dir and os.path.exists(save_dir):
                profile_path = os.path.join(
                    save_dir, f"profile_{profile_name}.png")
            else:
                # Fallback to temp directory
                temp_dir = tempfile.gettempdir()
                profile_path = os.path.join(
                    temp_dir, f"profile_{profile_name}.png")

        # Debug: check if file exists
        QgsMessageLog.logMessage(
            f"Looking for profile at: {profile_path}", "ClipRasterLayout", Qgis.Info)
        QgsMessageLog.logMessage(
            f"File exists: {os.path.exists(profile_path)}", "ClipRasterLayout", Qgis.Info)

        # List all profile files in directory where we're looking
        profile_dir = os.path.dirname(profile_path)
        if os.path.exists(profile_dir):
            QgsMessageLog.logMessage(
                f"Available profile files in {profile_dir}:", "ClipRasterLayout", Qgis.Info)
            for file in os.listdir(profile_dir):
                if file.startswith("profile_") and file.endswith(".png"):
                    QgsMessageLog.logMessage(
                        f"  - {file}", "ClipRasterLayout", Qgis.Info)
        ...
        if os.path.exists(profile_path):
            ...
            QgsMessageLog.logMessage(
                "Profile image added to layout", "ClipRasterLayout", Qgis.Info)
        else:
            QgsMessageLog.logMessage(
                f"Profile image not found at {profile_path}", "ClipRasterLayout", Qgis.Warning)
    ...
    def add_metadata_table(self, layout, raster_layer, page_size):
        ...
        metadata_box.attemptMove(QgsLayoutPoint(
            frame_x, frame_y, QgsUnitTypes.LayoutMillimeters))
        metadata_box.attemptResize(QgsLayoutSize(
            frame_width, frame_height, QgsUnitTypes.LayoutMillimeters))
        metadata_box.setFrameEnabled(True)
        metadata_box.setFrameStrokeWidth(
        ...
        metadata = [
            ...
            ("Dimensioni:",
             f"{raster_layer.width()} x {raster_layer.height()} px")
        ]
    ...
    def export_pdf(self):
        ...
        if file_path:
            ...
            if result == QgsLayoutExporter.Success:
                QMessageBox.information(
                    self, "Success", "PDF exported successfully!")
            else:
    ...
    def export_atlas_pdf(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Atlas PDF", "", "PDF Files (*.pdf)")
        if not file_path:
            return
        ...
        try:
            ...
            if result == QgsLayoutExporter.Success:
                ...
                QMessageBox.warning(
                    self, "Error", f"Error exporting Atlas PDF: {result}")

        except Exception as e:
            QMessageBox.warning(
                self, "Error", f"Error exporting Atlas: {str(e)}")
    ...
    def generate_atlas_layout(self, raster_layer):
        ...
        try:
            ...
            QgsMessageLog.logMessage(
                f"Found {feature_count} sections in {profile_layer.name()}", "ClipRasterLayout", Qgis.Info)
            ...
            map_item.attemptMove(QgsLayoutPoint(
                10, 40, QgsUnitTypes.LayoutMillimeters))
            map_item.attemptResize(QgsLayoutSize(
                size[0] * 0.65, size[1] * 0.5, QgsUnitTypes.LayoutMillimeters))
            ...
            title = QgsLayoutItemLabel(layout)
            title.setText(
                f"[% '{self.title_edit.text()}' %] - Section [% \"name\" %]")
            title.setFont(QFont("Arial", 18, QFont.Bold))
            title.setHAlign(Qt_AlignHCenter)
            title.attemptMove(QgsLayoutPoint(
                10, 10, QgsUnitTypes.LayoutMillimeters))
            title.attemptResize(QgsLayoutSize(
                size[0] - 20, 15, QgsUnitTypes.LayoutMillimeters))
            layout.addLayoutItem(title)
            ...
            info_label.attemptMove(QgsLayoutPoint(
                size[0] * 0.7, 40, QgsUnitTypes.LayoutMillimeters))
            info_label.attemptResize(QgsLayoutSize(
                size[0] * 0.25, 60, QgsUnitTypes.LayoutMillimeters))
            info_label.setFrameEnabled(True)
            info_label.setFrameStrokeWidth(
                QgsLayoutMeasurement(0.3, QgsUnitTypes.LayoutMillimeters))
            ...
            if save_dir:
                ...
                profile_pic.setDataDefinedProperty(
                    QgsLayoutObject.PictureSource,
                    QgsProperty.fromExpression(
                        f"'{temp_dir}/profile_' || \"name\" || '.png'")
                )

            profile_pic.attemptMove(QgsLayoutPoint(
                10, size[1] * 0.6, QgsUnitTypes.LayoutMillimeters))
            profile_pic.attemptResize(QgsLayoutSize(
                size[0] - 20, size[1] * 0.3, QgsUnitTypes.LayoutMillimeters))
            profile_pic.setFrameEnabled(True)
            profile_pic.setFrameStrokeWidth(
                QgsLayoutMeasurement(0.5, QgsUnitTypes.LayoutMillimeters))
            profile_pic.setResizeMode(QgsLayoutItemPicture.Zoom)
            ...
            if self.include_north.isChecked():
                ...
                north.setPicturePath(
                    ":/images/north_arrows/layout_default_north_arrow.svg")
                north.attemptMove(QgsLayoutPoint(
                    size[0] - 30, 40, QgsUnitTypes.LayoutMillimeters))
                north.attemptResize(QgsLayoutSize(
                    15, 20, QgsUnitTypes.LayoutMillimeters))
                north.setFrameEnabled(True)
                layout.addLayoutItem(north)
            ...
            if self.include_scale.isChecked():
                ...
                scalebar.setStyle('Single Box')
                scalebar.attemptMove(QgsLayoutPoint(
                    10, size[1] * 0.55, QgsUnitTypes.LayoutMillimeters))
                scalebar.setFrameEnabled(True)
                layout.addLayoutItem(scalebar)
            ...
            if self.include_metadata.isChecked():
                ...
                metadata_label.attemptMove(QgsLayoutPoint(
                    size[0] * 0.7, size[1] - 40, QgsUnitTypes.LayoutMillimeters))
                metadata_label.attemptResize(QgsLayoutSize(
                    size[0] * 0.25, 30, QgsUnitTypes.LayoutMillimeters))
            ...
            QgsMessageLog.logMessage(
                f"Error creating Atlas layout: {str(e)}", "ClipRasterLayout", Qgis.Critical)
            QMessageBox.warning(
    ...
    def load_template_layout(self, template_path, layout_name):
        ...
        try:
            ...
            if not manager.addLayout(layout):
                QgsMessageLog.logMessage(
                    "Could not add empty layout", "ClipRasterLayout", Qgis.Critical)
                return None
            ...
            if not doc.setContent(template_content):
                QgsMessageLog.logMessage(
                    "Failed to parse template XML", "ClipRasterLayout", Qgis.Critical)
            ...
            if not layout:
                QgsMessageLog.logMessage(
                    "Could not retrieve layout from manager", "ClipRasterLayout", Qgis.Critical)
                return None
            ...
            if not layout.readLayoutXml(doc.documentElement(), doc, context):
                QgsMessageLog.logMessage(
                    "Failed to read layout XML", "ClipRasterLayout", Qgis.Critical)
                manager.removeLayout(layout)
                return None

            QgsMessageLog.logMessage(
                f"Successfully loaded template into layout: {layout_name}", "ClipRasterLayout", Qgis.Info)

            return layout

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error loading template: {str(e)}", "ClipRasterLayout", Qgis.Critical)
            import traceback
            QgsMessageLog.logMessage(
                traceback.format_exc(), "ClipRasterLayout", Qgis.Critical)
            return None
        ...
        try:
            ...
            for item in layout.items():
                if isinstance(item, QgsLayoutItemMap):
                    item_id = item.id()
                    QgsMessageLog.logMessage(
                        f"Found map with ID: '{item_id}' on page {item.page()}, size: {item.rect().width()}x{item.rect().height()}", "ClipRasterLayout", Qgis.Info)

                    # Check by ID
                    if item_id.lower() == "map":
                        main_map = item
                        QgsMessageLog.logMessage(
                            "Identified as main map by ID", "ClipRasterLayout", Qgis.Info)
                    elif item_id.lower() == "map2" or "overview" in item_id.lower():
                        overview_map = item
                        QgsMessageLog.logMessage(
                            "Identified as overview map by ID", "ClipRasterLayout", Qgis.Info)
                    # If no clear ID, use size heuristic on page 0
                        ...
                        if item.rect().width() > 150:  # Larger maps are usually main maps
                            if not main_map:
                                main_map = item
                                QgsMessageLog.logMessage(
                                    "Identified as main map by size", "ClipRasterLayout", Qgis.Info)
                        else:
                            if not overview_map:
                                overview_map = item
                                QgsMessageLog.logMessage(
                                    "Identified as overview map by size", "ClipRasterLayout", Qgis.Info)
            ...
            if main_map:
                ...
                if profile_layer:
                    ...
                    QgsMessageLog.logMessage(
                        "Added section labels to main map", "ClipRasterLayout", Qgis.Info)
            ...
            QgsMessageLog.logMessage(
                f"Error populating template: {str(e)}", "ClipRasterLayout", Qgis.Critical)
    ...
    def populate_elevation_profiles(self, layout):
        ...
        try:
            ...
            if not profile_features:
                QgsMessageLog.logMessage(
                    "No profiles found to add to layout", "ClipRasterLayout", Qgis.Info)
                return
            ...
            for item in layout.items():
                if HAS_ELEVATION_PROFILE and isinstance(item, QgsLayoutItemElevationProfile):
                    ...
                    if item.page() == 1:  # Page index starts at 0
                        ...
                        QgsMessageLog.logMessage(
                            f"Found elevation profile item: {item.id()} at position {item.positionWithUnits()}", "ClipRasterLayout", Qgis.Info)

            # If no elevation profile items found, look for picture placeholders
            if not profile_items:
                QgsMessageLog.logMessage(
                    "No elevation profile items found, looking for picture placeholders", "ClipRasterLayout", Qgis.Info)
                ...
                for item in layout.items():
                    if isinstance(item, QgsLayoutItemPicture):
                        ...
                        if item.page() == 1:  # Page index starts at 0
                            ...
                            QgsMessageLog.logMessage(
                                f"Found picture item: {item.id()}", "ClipRasterLayout", Qgis.Info)

                if picture_items:
                    QgsMessageLog.logMessage(
                        f"Found {len(picture_items)} picture items for profiles on page 2", "ClipRasterLayout", Qgis.Info)
                    # Sort picture items by their position on page (top to bottom, left to right)
                    picture_items.sort(key=lambda x: (
                        x.positionWithUnits().y(), x.positionWithUnits().x()))
                    ...
                    for i, (pic_item, profile_feature) in enumerate(zip(picture_items[:len(profile_features)], profile_features)):
                        ...
                        if os.path.exists(profile_path):
                            ...
                            QgsMessageLog.logMessage(
                                f"Set profile image {i+1} to {profile_path}", "ClipRasterLayout", Qgis.Info)

                            # Add title label above the profile
                            self.add_profile_title(
                                layout, pic_item, f"Profilo {profile_name}")
                        else:
                            QgsMessageLog.logMessage(
                                f"Profile image not found: {profile_path}", "ClipRasterLayout", Qgis.Warning)
                    ...
                    for i in range(len(profile_features), len(picture_items)):
                        ...
                        QgsMessageLog.logMessage(
                            f"Hidden unused profile picture {i+1}", "ClipRasterLayout", Qgis.Info)
                return

            QgsMessageLog.logMessage(
                f"Found {len(profile_items)} elevation profile items on page 2", "ClipRasterLayout", Qgis.Info)
            QgsMessageLog.logMessage(
                f"Have {len(profile_features)} profile features to populate", "ClipRasterLayout", Qgis.Info)

            # Sort profile items by position (top to bottom, left to right)
            profile_items.sort(key=lambda x: (
                x.positionWithUnits().y(), x.positionWithUnits().x()))
            ...
            for i, profile_feature in enumerate(profile_features):
                if i >= len(profile_items):
                    QgsMessageLog.logMessage(
                        f"No more profile items available for feature {i+1}", "ClipRasterLayout", Qgis.Warning)
                    break
                ...
                try:
                    ...
                    profile_ready, _ = QgsProject.instance().readEntry(
                        "ClipRasterLayout", f"profile_ready_{profile_name}")

                    if profile_ready == "yes":
                        QgsMessageLog.logMessage(
                            f"Profile {profile_name} was marked as ready", "ClipRasterLayout", Qgis.Info)
                    ...
                    for dock in self.iface.mainWindow().findChildren(QDockWidget):
                        ...
                        if 'elevation' in dock_title.lower() and 'profile' in dock_title.lower():
                            ...
                            if profile_name in dock_title:
                                profile_dock = dock
                                QgsMessageLog.logMessage(
                                    f"Found specific dock for {profile_name}", "ClipRasterLayout", Qgis.Info)
                                break
                        ...
                        try:
                            ...
                            if elevation_canvas:
                                ...
                                if hasattr(profile_item, 'copyFromProfileCanvas'):
                                    profile_item.copyFromProfileCanvas(
                                        elevation_canvas)
                                    QgsMessageLog.logMessage(
                                        f"Copied profile {profile_name} using copyFromProfileCanvas", "ClipRasterLayout", Qgis.Info)
                                elif hasattr(profile_item, 'copyFromProfileWidget'):
                                    profile_item.copyFromProfileWidget(
                                        elevation_canvas)
                                    QgsMessageLog.logMessage(
                                        f"Copied profile {profile_name} using copyFromProfileWidget", "ClipRasterLayout", Qgis.Info)
                                else:
                                    # Fallback: set the curve manually
                                    self.setup_profile_item_manually(
                                        profile_item, profile_feature)
                                    QgsMessageLog.logMessage(
                                        f"Set profile {profile_name} manually (no copy method available)", "ClipRasterLayout", Qgis.Info)
                            else:
                                self.setup_profile_item_manually(
                                    profile_item, profile_feature)
                                QgsMessageLog.logMessage(
                                    f"Could not find elevation canvas in dock for {profile_name}", "ClipRasterLayout", Qgis.Warning)
                        except Exception as e:
                            QgsMessageLog.logMessage(
                                f"Error copying from profile dock: {str(e)}", "ClipRasterLayout", Qgis.Warning)
                            self.setup_profile_item_manually(
                                profile_item, profile_feature)
                    else:
                        # No profile dock found or not visible, set curve manually
                        QgsMessageLog.logMessage(
                            f"No visible profile dock for {profile_name}, setting up manually", "ClipRasterLayout", Qgis.Info)
                        self.setup_profile_item_manually(
                            profile_item, profile_feature)
                    ...
                    QgsMessageLog.logMessage(
                        f"Populated profile {i+1} with {profile_name}", "ClipRasterLayout", Qgis.Info)

                except Exception as e:
                    QgsMessageLog.logMessage(
                        f"Error setting profile {i+1}: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            ...
            for i in range(len(profile_features), len(profile_items)):
                ...
                QgsMessageLog.logMessage(
                    f"Hidden unused profile {i+1}", "ClipRasterLayout", Qgis.Info)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error populating elevation profiles: {str(e)}", "ClipRasterLayout", Qgis.Critical)
    ...
    def setup_profile_item_manually(self, profile_item, profile_feature):
        ...
        try:
            ...
            if dem_layer:
                try:
                    ...
                    try:
                        ...
                        QgsMessageLog.logMessage(
                            "Could not configure layers for profile", "ClipRasterLayout", Qgis.Warning)
            ...
            QgsMessageLog.logMessage(
                f"Error setting up profile manually: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            ...
            for feature in profile_layer.getFeatures():
                ...
                if geometry.type() == QgsWkbTypes.LineGeometry:
                    ...
                    if len(line) >= 2:
                        ...
                        start_layout = map_item.mapToItemCoords(
                            QPointF(start_point.x(), start_point.y()))
                        end_layout = map_item.mapToItemCoords(
                        ...
                        start_label.setBackgroundColor(
                            QColor(255, 255, 255))  # White background
                        start_label.setFrameEnabled(True)
                        start_label.setFrameStrokeColor(
                            QColor(0, 0, 0))  # Black border
                        start_label.setFrameStrokeWidth(
                            QgsLayoutMeasurement(0.5, QgsUnitTypes.LayoutMillimeters))
                        ...
                        start_label.attemptResize(QgsLayoutSize(
                            label_size, label_size, QgsUnitTypes.LayoutMillimeters))
                        ...
                        end_label.setBackgroundColor(
                            QColor(255, 255, 255))  # White background
                        end_label.setFrameEnabled(True)
                        end_label.setFrameStrokeColor(
                            QColor(0, 0, 0))  # Black border
                        end_label.setFrameStrokeWidth(QgsLayoutMeasurement(
                            0.5, QgsUnitTypes.LayoutMillimeters))
                        ...
                        end_label.attemptResize(QgsLayoutSize(
                            label_size, label_size, QgsUnitTypes.LayoutMillimeters))
                        ...
                        QgsMessageLog.logMessage(
                            f"Added labels for section {profile_name}", "ClipRasterLayout", Qgis.Info)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error adding section labels: {str(e)}", "ClipRasterLayout", Qgis.Warning)
    ...
    def add_profile_title(self, layout, profile_item, title):
        ...
        try:
            ...
            QgsMessageLog.logMessage(
                f"Added title '{title}' to profile", "ClipRasterLayout", Qgis.Info)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error adding profile title: {str(e)}", "ClipRasterLayout", Qgis.Warning)
