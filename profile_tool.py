...
from qgis.core import (QgsPointXY, QgsGeometry, QgsFeature,
                       QgsVectorLayer, QgsProject, QgsWkbTypes, QgsCoordinateTransform,
                       QgsRasterLayer, QgsLineString, QgsPoint,
                       QgsRaster, QgsSimpleLineSymbolLayer, QgsTextAnnotation, QgsMessageLog,
                       Qgis)
...
from qgis.gui import QgsRubberBand, QgsMapTool, QgsMapCanvasAnnotationItem
import numpy as np
import math
...
class ProfileTool(QgsMapTool):
    ...
    def canvasPressEvent(self, event):
        ...
        if self.start_point is None:
            ...
            # Calculate 2D length
            line = QgsLineString(
                [QgsPoint(self.start_point), QgsPoint(end_point)])
            ...
            feature.setAttributes([self.profile_count, letter_pair, float(
                length_2d), 0.0, float(elev_a), float(elev_b)])

            # Add to layer and get the new feature id
            success, features = self.profile_layer.dataProvider().addFeatures([
                feature])
            ...
            QgsProject.instance().writeEntry("ClipRasterLayout",
                                             f"profile_geom_{letter_pair}", feature.geometry().asWkt())
    ...
    def add_labels(self, start_point, end_point, letter_pair):
        ...
        start_annotation.setMapPositionCrs(
            self.canvas.mapSettings().destinationCrs())
        ...
        end_annotation.setMapPositionCrs(
            self.canvas.mapSettings().destinationCrs())
            ...
            if hasattr(self, 'current_profile_info') and self.current_profile_info:
                ...
                for feature in self.profile_layer.getFeatures():
                    if feature['id'] == self.current_profile_info['profile_count']:
                        ...
                        self.profile_layer.changeAttributeValue(
                            feature.id(), 3, float(length_3d))
                        self.profile_layer.commitChanges()
                        QgsMessageLog.logMessage(
                            f"Updated profile {feature['name']} with 3D length: {length_3d:.2f}m", "ClipRasterLayout", Qgis.Info)
                        break
    ...
    def create_profile_plot(self, distances, elevations, name):
        ...
        ax.set_title(
            f'Profilo Topografico {name}', fontsize=14, fontweight='bold')
        ...
        QgsMessageLog.logMessage(
            f"Profile saved to: {profile_path}", "ClipRasterLayout", Qgis.Info)
        QgsMessageLog.logMessage(
            f"File exists: {os.path.exists(profile_path)}", "ClipRasterLayout", Qgis.Info)

        # Store profile data
        profile_data = {'name': name, 'figure': fig, 'distances': distances,
                        'elevations': elevations, 'image_path': profile_path}
        self.profiles.append(profile_data)

        # Store profile path in project custom properties
        QgsProject.instance().writeEntry("ClipRasterLayout",
                                         f"profile_{name}", profile_path)
        ...
        try:
            if self.profile_dock is None:
                ...
                self.iface.addDockWidget(
                    Qt.LeftDockWidgetArea, self.profile_dock)
                self.profile_dock.show()
                QgsMessageLog.logMessage(
                    "Created main profile dock widget", "ClipRasterLayout", Qgis.Info)
            ...
            QgsMessageLog.logMessage(
                f"Added profile tab for {name}", "ClipRasterLayout", Qgis.Info)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Failed to create/update dock widget: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            # Fallback to dialog
            try:
                dlg = ProfileDialog(fig, name)
                dlg.show()
                QgsMessageLog.logMessage(
                    f"Created profile dialog for {name} as fallback", "ClipRasterLayout", Qgis.Info)
            except Exception as e2:
                QgsMessageLog.logMessage(
                    f"Failed to create profile dialog: {str(e2)}", "ClipRasterLayout", Qgis.Critical)
...
class ProfileSaveDialog(QDialog):
    ...
    def setupUi(self):
        ...
        layout.addWidget(
            QLabel("Seleziona dove salvare le immagini dei profili:"))
...
class ProfileTabDockWidget(QDockWidget):
    def __init__(self, iface, parent=None):
        ...
        all_profiles_btn.setToolTip(
            "Prepara i profili elevazione per l'uso nel layout")
        ...
        try:
            ...
            for action in actions:
                if 'elevation' in action.text().lower() and 'profile' in action.text().lower():
                    ...
                    opened = True
                    QgsMessageLog.logMessage(
                        "Triggered elevation profile action", "ClipRasterLayout", Qgis.Info)
                    break
                        ...
                        if action.menu():  # Panels submenu
                            for subaction in action.menu().actions():
                                if 'elevation' in subaction.text().lower() and 'profile' in subaction.text().lower():
                                    ...
                                    opened = True
                                    QgsMessageLog.logMessage(
                                        "Triggered elevation profile through View menu", "ClipRasterLayout", Qgis.Info)
                                    break

            if not opened:
                QgsMessageLog.logMessage(
                    "Could not find elevation profile action", "ClipRasterLayout", Qgis.Warning)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error opening elevation profile: {str(e)}", "ClipRasterLayout", Qgis.Warning)
    ...
    def prepare_profiles_for_layout(self):
        ...
        try:
            ...
            QgsMessageLog.logMessage(
                "Starting sequential profile creation...", "ClipRasterLayout", Qgis.Info)
            ...
            if not profile_layer:
                QgsMessageLog.logMessage(
                    "Profile layer not found", "ClipRasterLayout", Qgis.Warning)
                return

            # Get all profile features
            self.profile_features_to_process = list(
                profile_layer.getFeatures())
            self.current_profile_index = 0

            if not self.profile_features_to_process:
                QgsMessageLog.logMessage(
                    "No profiles found to process", "ClipRasterLayout", Qgis.Warning)
                return
            ...
            QgsMessageLog.logMessage(
                f"Error preparing profiles: {str(e)}", "ClipRasterLayout", Qgis.Warning)
    ...
    def process_next_profile(self):
        ...
        try:
            ...
            if self.current_profile_index >= len(self.profile_features_to_process):
                ...
                QgsMessageLog.logMessage(
                    f"Completed processing {len(self.profile_features_to_process)} profiles", "ClipRasterLayout", Qgis.Info)
            ...
            QgsMessageLog.logMessage(
                f"Processing profile {self.current_profile_index + 1}/{len(self.profile_features_to_process)}: {profile_name}", "ClipRasterLayout", Qgis.Info)
            ...
            QTimer.singleShot(
                1000, lambda: self.configure_current_profile(feature))

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error processing profile: {str(e)}", "ClipRasterLayout", Qgis.Warning)
    ...
    def configure_current_profile(self, feature):
        ...
        try:
            ...
            if elevation_dock:
                ...
                elevation_dock.setWindowTitle(
                    f"Profilo Elevazione {profile_name}")
                QgsMessageLog.logMessage(
                    f"Renamed profile dock from '{old_title}' to: 'Profilo Elevazione {profile_name}'", "ClipRasterLayout", Qgis.Info)
                ...
                try:
                    ...
                    if tree_widgets:
                        tree = tree_widgets[0]
                        QgsMessageLog.logMessage(
                            f"Found layer tree widget with {tree.topLevelItemCount()} items", "ClipRasterLayout", Qgis.Info)
                        ...
                        if self.dem_layer:
                            ...
                            for i in range(tree.topLevelItemCount()):
                                ...
                                if item.text(0) == dem_name:
                                    ...
                                    QgsMessageLog.logMessage(
                                        f"Enabled DEM layer: {dem_name}", "ClipRasterLayout", Qgis.Info)
                                    break
                except Exception as e:
                    QgsMessageLog.logMessage(
                        f"Could not configure layer checkboxes: {str(e)}", "ClipRasterLayout", Qgis.Warning)
                ...
                if elevation_canvas and HAS_ELEVATION_PROFILE:
                    try:
                        ...
                        if geometry and geometry.type() == QgsWkbTypes.LineGeometry:
                            elevation_canvas.setProfileCurve(
                                geometry.constGet())
                            QgsMessageLog.logMessage(
                                f"Set profile curve for {profile_name}", "ClipRasterLayout", Qgis.Info)

                            # Give time for the curve to be set
                            QTimer.singleShot(500, lambda: self.capture_profile_curve(
                                elevation_dock, feature))
                            return  # Don't process next profile yet
                    except Exception as e:
                        QgsMessageLog.logMessage(
                            f"Could not set profile curve: {str(e)}", "ClipRasterLayout", Qgis.Warning)

                # If we couldn't set up the curve automatically, show message and continue
                self.show_profile_ready_message(profile_name)
            else:
                QgsMessageLog.logMessage(
                    "Elevation profile dock not found", "ClipRasterLayout", Qgis.Warning)
            ...
            QgsMessageLog.logMessage(
                f"Error configuring profile: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            self.current_profile_index += 1
            self.process_next_profile()
    ...
    def capture_profile_curve(self, elevation_dock, feature):
        ...
        try:
            ...
            QgsMessageLog.logMessage(
                f"Found {len(all_buttons)} tool buttons in elevation dock", "ClipRasterLayout", Qgis.Info)

            for button in all_buttons:
                tooltip = button.toolTip()
                QgsMessageLog.logMessage(
                    f"Button tooltip: '{tooltip}'", "ClipRasterLayout", Qgis.Info)
                ...
                if ('feature' in tooltip_lower) or \
                   ...
                   ('from selected' in tooltip_lower):
                    ...
                    QgsMessageLog.logMessage(
                        f"Selected capture button with tooltip: {tooltip}", "ClipRasterLayout", Qgis.Info)
                    break
            ...
            if capture_button:
                ...
                if profile_layer:
                    ...
                    QgsMessageLog.logMessage(
                        f"Selected feature {profile_name} in profile layer", "ClipRasterLayout", Qgis.Info)
                    ...
                    if capture_button.isCheckable():
                        ...
                        QgsMessageLog.logMessage(
                            f"Toggled 'capture curve from feature' button ON for {profile_name}", "ClipRasterLayout", Qgis.Info)
                    else:
                        capture_button.click()
                        QgsMessageLog.logMessage(
                            f"Clicked 'capture curve from feature' button for {profile_name}", "ClipRasterLayout", Qgis.Info)
                    ...
                    QTimer.singleShot(
                        1500, lambda: self.finish_profile_configuration(profile_name))
                else:
                    self.show_profile_ready_message(profile_name)
            else:
                QgsMessageLog.logMessage(
                    "Could not find 'capture curve from feature' button", "ClipRasterLayout", Qgis.Warning)
                # Log all button tooltips for debugging
                for button in elevation_dock.findChildren(QToolButton):
                    if button.toolTip():
                        QgsMessageLog.logMessage(
                            f"Button tooltip: {button.toolTip()}", "ClipRasterLayout", Qgis.Info)
                self.show_profile_ready_message(profile_name)

        except Exception as e:
            QgsMessageLog.logMessage(
                f"Error capturing profile curve: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            self.show_profile_ready_message(profile_name)
    ...
    def finish_profile_configuration(self, profile_name):
        ...
        try:
            ...
            QgsProject.instance().writeEntry("ClipRasterLayout",
                                             f"profile_ready_{profile_name}", "yes")
            QgsMessageLog.logMessage(
                f"Profile {profile_name} configuration completed", "ClipRasterLayout", Qgis.Info)
            ...
            QgsMessageLog.logMessage(
                f"Error finishing profile configuration: {str(e)}", "ClipRasterLayout", Qgis.Warning)
            self.show_profile_ready_message(profile_name)

    def show_profile_ready_message(self, profile_name):
        """Show message that profile is ready and move to next"""
        QMessageBox.information(None, f"Profilo {profile_name}",
